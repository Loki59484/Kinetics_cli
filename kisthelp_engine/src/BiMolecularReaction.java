...
public class BiMolecularReaction extends ElementaryReaction implements SessionComponent, ReadWritable {
    ...
    public static void write2DArraytoCSV(double[][] data, String outFile, String header) throws IOException,IllegalDataException {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(outFile))) {
            ...
            if (data == null || data.length != 2 || data[0].length != data[1].length) {
                System.err.println("Input data must be a 2xN array.");
            ...
            for (int i = 0; i < numPoints; i++) {
                ...
                String line = String.format(Locale.US,"%g,%e", xValue, yValue);
                writer.write(line);
                writer.newLine();
            }
            System.out.println(outFile+" was written successfully.");
        }
    ...
    public Vector getGraphicsResults(TemperatureRange temperatureRange) throws runTimeException, IllegalDataException {
        ...
        if (cli) {
            try {
                Path dirpath = Paths.get(Kistep.getUserCurrentDirectory()+"/Results/");
                String TemporaryFilename = workfile.getName();
                Files.createDirectories(dirpath);
                System.out
                        .println("All plotting data will be written to csv files.");
                write2DArraytoCSV(KArray, dirpath+ "/" +  TemporaryFilename.replace(".kinp", "kTST"), "Temp,kTST");
                write2DArraytoCSV(KArray2,dirpath+ "/" +  TemporaryFilename.replace(".kinp", "kTST_tunnelling"), "Temp,kTST_tunnel");
                write2DArraytoCSV(KArray3,dirpath+ "/" +  TemporaryFilename.replace(".kinp", "kVTST"), "Temp,kVTST");
                write2DArraytoCSV(KArray4,dirpath+ "/" +  TemporaryFilename.replace(".kinp", "kVTST_tunnelling"), "Temp,kVTST_tunnel");
