...
public class ChemicalSystem implements ReadWritable {
    ...
    protected Complex[]  unscaledVibFreq, initialUnscaledVibFreq; // in K !! // depends on the session (via scaling factor)
    ...
    protected String     nature; // can be one of the static constants (minimum, saddlePoint1, pathPoint, anyPoint) 
...
// constructor 1 is used to build a chemical system that is not a reaction path point
// => the filename is required into this class because only molecular information is required
// => the possibility to directly read the data into a gaussian file (or GAMESS...) is given
...
// Session.getCurrentSession().getFilesToBeRead().getFirstAndRemove();
    ...
    public ChemicalSystem (String nature) throws CancelException, IllegalDataException, IOException {	
          ...
          openTestReadingFile();          // call to the openTestReadingFile method: open,test,read a file (contains the dataRead method) 
                                         //  in the ChemicalSystem class
          testCoherenceData();            // call to the testCoherenceData method: test the coherence of data in the ChemicalSystem class
...
/* C O N S T R U C T O R  2 , (Chemical system is to be read from a reactionpath file)*/
// constructor 2 is used to build a chemical system that belongs to a reaction path point
// => the filename has been required before (in the ReactionPath Class) because data (other than  molecular information) was required
// (i.e reaction coordinate along the reaction path, read in class ReactionPath)
// other chemical systems (other than the one presently read)  are included in this same data file,
// that is the reason why, in this case, the data file is already open and the "read" action is a parameter
// in this case, no possibility to directly read a gaussian file (or GAMESS ...) is given ...

    public ChemicalSystem (String nature, ActionOnFileRead read) throws IllegalDataException, IOException{	
      this.nature = nature;
 

	  dataRead(read, Keywords.endOfPointSection); 	// call to the dataRead method: to read the file.kinp (must be of KISTHEP type!) 	                                               
    ...
    public static File returnKinpFile(String nature, String question) throws CancelException, IllegalDataException, IOException {
    	...
    	// this method ask for a filename, and return a filled kinpFile describing a chemicalSystem
    ChemicalSystem temporarySystem = new ChemicalSystem(); // return an empty object here (constructor 3 of new ChemicalSystem)
...
//  ask for a filename, will serve as input, but can be either a .kinp or .out QM file 
    temporaryFileName = KisthepDialog.requireExistingFilename(question, new KisthepInputFileFilter(Constants.anyAllowedDataFile));
    

    // DETECT THE OUTPUT TYPE of this file (gaussian09, or GAMESS or NWchem or ORCA or MOLPRO or kinp); 
    ...
    // now, we are sure that the open file is ONLY of gaussian or GAMESS or NWchem or ORCA or MOLPRO or kinp file type !
    ...
    if (inputType.equals(Constants.kInpFileType))

     {
      	...
      	// duplicate this kinp file, because it will be erased at the end (in Interface/buildReactionPath)
        // and the user kinp file must not be erased
      	...
      	read.end();// close the read action on user kinp file
      	           // but all the properties of the readAction are still available through obkect "read" !
    ...
    temporarySystem.testCoherenceData(); // call to the testCoherenceData method: test the coherence of data in the ChemicalSystem class
...
/* M E T H O D S */
    ...
    if (atomic) {atomNb=1;}
  	      ...
  	      if (linear) {atomNb=(vibFreq.length+5)/3;}
  	      
  	      else {atomNb=(vibFreq.length+6)/3;}
    }
    ...
    if (!atomic) {
       ...
       for (int i=0; i<inertia.length; i++) {initialInertia[i]=inertia[i];}
       initialUnscaledVibFreq = new Complex[unscaledVibFreq.length];
       for (int i=0; i<unscaledVibFreq.length; i++) {initialUnscaledVibFreq[i] = (Complex) unscaledVibFreq[i].clone();}
...
for (int i=0; i<vibFreq.length;i++){
	if (hrdsBarrier[i] != Constants.highEnergy){nbHinder++;}
}
return nbHinder;
...
int nbImg = 0;
for (int i=0; i<vibFreq.length;i++){
	if (vibFreq[i].getImagPart()!=0){nbImg++;}
}
return nbImg;
} // end of getVibImg
...
int nbReal = 0;
for (int i=0; i<vibFreq.length;i++){
	if (vibFreq[i].getImagPart()==0){nbReal++;}
}
return nbReal;
} // end of getVibReal
...
// assuming one 1D K rotor: the one with the smallest inertia moment of a molecule with 3 inertia moments
...
// actually, KISTHELP manages RRKM rate constants only for non atomic and not LINEAR systems! 
// assuming one 2D rotor: the one corresponding to the  largest inertia moment averaging
...
if( inertia.length!=3){ 
	...
	message = message +  "KISTHELP manages RRKM rate constants only for non atomic and non LINEAR systems."+ Constants.newLine;
	JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage,JOptionPane.ERROR_MESSAGE);                  
	throw new runTimeException();}
...
// k=1 => sum of states
// r=the number of one dimensional rotors ("K rotor" + internal rotor eventually)
...
if((getVibFreedomDegrees()==0)){ 
	   ...
	   message = message + "attempt was made to calculate rovib. density of states for an atom " + Constants.newLine;
...
public double getNETot(double E, double T, double P) throws runTimeException{
	...
	// thus, N_tot = N_rov_vib_trans_elec = N_ro_vib_trans x P_elec(E) (P(E) = electronic degeneracy)
	// in fact, P_elec does not depend on E since only the ground state is considered here
	...
	if (atomic) {return getNETranslation(E,T,P);} // a single atom 
	
	if (inertia.length==1) {r=1;} // a linear molecule (can be with more than two atoms (HCN))
	else {r=3;}
...
public double getNETranslation(double E, double T, double P) throws runTimeException{
	...
	// a semi-classical approach is employed here, rather than the Laplace-transform method based on the inversion
	...
	// it depends on current pressure P and temperature T through the volume V present in the partition function initially

	// E must be given in Joule/molec; it makes reference to the zero of translational energy, being the 0 here.
	// it must be >0
	if (E < 0) {
	...
	density = density *  (Constants.R*T/P) /Constants.NA; // do not forget to convert to the volume occupied by one molecule
...
// it is the case in RRKM calculations where only the K-rotor (one-dimensional rotor) 
// is considered in the active degrees of freedom or for diatomic molecules
// else, in Kisthep, we can only consider r=3 if the vib+ global rotation (1 one-dimensional + 1 two-dimensional rotors) is needed	
// no hindered rotor are considered in this version for the density/sum of states calculation
...
// page 112, relations 6-92
	...
	if((getVibFreedomDegrees()==0)){ 
		   ...
		   String message = "Error in class ChemicalSystem in method getTstar" + Constants.newLine;            
		   message = message + "attempt was made to get to calculate rovib. density of states for an atom "+ Constants.newLine;
		   JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage,
				   JOptionPane.ERROR_MESSAGE);					        
	        throw new runTimeException();
	...
	// E must be given in Joule/molec; it makes reference to the zero of energy being the ZPE here.
...
// actually, KISTHEP manages RRKM rate constants only for non atomic and diatomic systems! 
	...
	if ((r != 1) && (r !=3))  {
		   ...
		   String message = "Error in class ChemicalSystem in method getNEWE " + Constants.newLine;            
		   message = message + "only density/sum of states calculation for (a one-dimensional, r=1) or (a one-dimensional + a two-dimensional rotors, r=3) is  accepted"+ Constants.newLine;
  ...
  double  tStar = getTStar(E,k,r); // relation 6-88 page 111 of "Theory of Unimolecular Reactions", Wendel Forst, Academic press
 
  
  // k = 1 for sum of states, = 0 for density of states
  // r= n1 + n2 the number of one- and two-dimensional rotors, respectively
  // in this program r will be 1 (K rotator) or 3 (K rotor + a two-dimensional J rotor)
  double  temporary;
  double  currentFrequency;
  double  density,tetPhi;

// calculate the density (or sum, depending on k=0 or k=1) of state at the energy E (J)
density = 1;
tetPhi = (k + (double)r/2.0)* Math.pow(Constants.kb*tStar,2);// relation 6-92 page 112 of "Theory of Unimolecular Reactions", Wendel Forst, Academic press
	     ...
	     for (int iFreq=0; iFreq< vibFreq.length; iFreq++){ //loop for scanning all the frequencies
		     ...
		     { // We test the value of frequencie to avoid imaginary frequencies
              ...
              // preparing numerator
			 currentFrequency = vibFreq[iFreq].getRealPart();  // we get the real part of complex 		
             density = density*1.0/(1.0- Math.exp(-currentFrequency/tStar) );	// relation 6-90 page 112 of "Theory of Unimolecular Reactions", Wendel Forst, Academic press
                                                                             // vib partition function is used, relative to ZPE !!
	        // preparing denominator
            // relation 6-92 page 112 of "Theory of Unimolecular Reactions", Wendel Forst, Academic press
        ...
        // but density/sum of states can be computed for the general case
	     // where 3 rotations (a one-dimensional and a two-dimensional rotors) can be considered: r=3 (r=n1+2n2)

	     // old version:
	     //density = density * Math.pow(8*Math.pow(Math.PI,3)/Math.pow(Constants.h,2),(double)r/2.0);          

	     // new version (22/04/2013)
	     density = density * Math.pow(8*Math.pow(Math.PI,2)/Math.pow(Constants.h,2),(double)r/2.0);    // relation 6-42 page 99 of "Theory of Unimolecular Reactions", Wendel Forst, Academic press   
         density = density * Math.pow(Math.PI, 0.5); // only one rotor is considered for the one-dimensional rotors in Kisthep (n1=1 always)(no hindered roors)
            		
          // get the one-dimensional rotor inertia moment
          double inertiaMoment1D = get1DRotorsInertia();
          inertiaMoment1D = inertiaMoment1D * Constants.convertAmuToKg*Math.pow(Constants.a0,2);    // convert inertia moment from amu.bohr^2 to kg.m^2 
          density = density * Math.pow(inertiaMoment1D,0.5); // for the "K rotor"

          //IF and ONLY IF r=3 (all rotations are required for density/sum of state calculation), include the 
        	  //contribution of the two-dimensional rotor in the inertia moment contribution
        	  //(the other contribution of this two-dimensional rotor are already included via "r" in the previous formula)
          
          if (r==3) {
          // get the two-dimensional rotor inertia moment        	  
          double inertiaMoment2D = get2DRotorsInertia();
         
          inertiaMoment2D = inertiaMoment2D * Constants.convertAmuToKg*Math.pow(Constants.a0,2);    // convert inertia moment from amu.bohr^2 to kg.m^2 
          density = density * inertiaMoment2D; // not that for the "two-dimensional J rotor", the square root of inertia moment is removed 
          } // end of if r==3
...
/**********************************************/    // OUTPUT TYPE (KISTHEP, GAUSSIAN, GAMESS, ORCA  ...?),   READS IT, AND 
   ...
   public void openTestReadingFile () throws IllegalDataException, IOException, CancelException { 
       ...
       ActionOnFileRead read; 
       File temporyFileName;
       String question = Constants.askingFileString+Session.getCurrentSession().getFilesToBeRead().getFirstAndRemove();

//     ask for a filename 
       temporyFileName = KisthepDialog.requireExistingFilename(question, new KisthepInputFileFilter(Constants.anyAllowedDataFile));
       

       // pay attention: an absolute path is used below ; and DETECT THE OUTPUT TYPE (gaussian09, or GAMESS or NWchem or ORCA or MOLPRO or kinp)
...
// read into a KISTHEP type file and check if String read are decimal numbers, or integer, ...
    
    public  void dataRead(ActionOnFileRead read, String endOfSection) throws IllegalDataException, IOException { 
    
    // end of section can be endOfFile (a simple kinp file is read) or endOfPointSection  (a reaction path is read)
    	String currentLine;
    	currentLine = read.oneString(); 
    
	
    // reading loop      
      if (currentLine == null) {currentLine = Keywords.endOfFile;}       
	...
	{ 
     ...
     //	endOfSection=endOfPathPoint, if currentLine == null is met before **END, user forgot to put **END ...
	 if (currentLine == null) {currentLine = Keywords.endOfFile;}
	 if ( (currentLine.equals(Keywords.endOfFile)) && (endOfSection.equals(Keywords.endOfPointSection))) 
	    {		       
	       ...
	       message = message + "while reading a reactionPathPoint section in file " + read.getWorkFile() + Constants.newLine;
    ...
    /**********************************/
    /* k i n p F r o m M O L P R O */   // build a kinp input file from data in a MOLPRO (from version 2015.1) frequency outputfile  
    /********************************/   
        
        public  static void kinpFromMOLPRO(ActionOnFileRead read, ActionOnFileWrite write) throws IllegalDataException, IOException { 
     ...
     // molecular properties 
        ...
        String[] massBuffer, energyBuffer, symNumberBuffer;
    	    ...
    	    boolean  tagFreqImagin=false, tagFreqReal=false, tagFreqLow=false, flagGetXYZ=false, flagTypeAtom=false;
    	    
    	    double[][] inertiaTensor = new double[3][3]; // inertia tensor of the molecule in order to compute the inertia moments
        ...
        Matcher mJob, mMult, mFreqLow, mFreqReal, mSym, mTypeAtom;
         ...
         /* FIRST: filtering the MOLPRO output file   */ 
    	    /************************************************/ 
        symNumber = "1"; // !! MOLPRO supplies the symmetry number only if the thermo analysis has been required; 
        ...
        while (currentLine != null) {
	 	         ...
	 	         if (mMult.find()) {
	 	             ...
	 	             degen= buffer[10];
	 	        	     ...
	 	        	     if (degen.equals("Singlet")) {degen="1";}
	 	        	     else if (degen.equals("Doublet")) {degen="2";}
	 	        	     else if (degen.equals("Triplet")) {degen="3";}
	 	        	     else if (degen.equals("Quartet")) {degen="4";}
	 	        	     else if (degen.equals("Quintet")) {degen="5";}
	 	        	     else if (degen.equals("sextet")) {degen="6";}
	 	        	     else if (degen.equals("septet")) {degen="7";}
	 	       	         else if (degen.equals("octet")) {degen="8";}
	 	        	     else if (degen.equals("nonet")) {degen="9";}
	 	        	     else degen="";
	 	         }
                 ...
                 /* IMAGINARY VIBRATIONAL FREQUENCIES   (in MOLPRO, real and imaginary frequencies are reported in two different parts)   */    
    		mFreqImagin=pFreqImagin.matcher(currentLine);			
    		if (mFreqImagin.find()) {tagFreqImagin=true; tagFreqReal=false;}
    		...
    		if (tagFreqImagin && mFreqValue.find()) {  		   		
            	      ...
            	      for (int iFreq = 3; iFreq <= freqBuffer.length-1; iFreq++) {             		       
    		               ...
    		               message = message + "while reading vibrational frequencies in file " + read.getWorkFile() + Constants.newLine;
        ...
        /* LOW mode VIBRATIONAL FREQUENCIES   (in MOLPRO, real and imaginary frequencies are reported in two different parts)   */  
    		/* low modes : end of reading real or imaginary frequencies */
    		
		mFreqLow=pFreqLow.matcher(currentLine);			
		if (mFreqLow.find()) {tagFreqImagin=false; tagFreqReal=false; tagFreqLow=true; ilow=0;}
		...
		if (tagFreqLow && mFreqValue.find()) {  		   		
	   	      ...
	   	      for (int iFreq = 3; iFreq <= freqBuffer.length-1; iFreq++) {
		               ...
		               /* in MOLPRO, the section " Normal Modes of low/zero frequencies"
		                may contain  6 or more low values ! When 7 or more values are read, the values beyond
		                6 corresponds likely to hindered rotors and have to be taken in the set of vib frequencies*/
		               if (ilow>6) {
        ...
        /* REAL VIBRATIONAL FREQUENCIES   (in MOLPRO, real and imaginary frequencies are reported in two different parts)   */    
	mFreqReal=pFreqReal.matcher(currentLine);			
	if (mFreqReal.find() && (!tagFreqImagin) && (!tagFreqLow) ) {tagFreqReal=true;}
	...
	if (tagFreqReal && mFreqValue.find()) {  		   		
   	      ...
   	      for (int iFreq = 3; iFreq <= freqBuffer.length-1; iFreq++) {             		       
                 ...
                 /* MASS */    
    		// in MOLPRO, since it is mandatory to read cartesian coordinates with atom masses
    		// the total mass can be obtained from the atom mass sum using data stored in Kisthelp
    		// --> for an atom, there is no need to perform a frequency calculation within MOLPRO
    	    // atom type or molecule ?
	    mTypeAtom=pTypeAtom.matcher(currentLine);			
     	if  (mTypeAtom.find()) {flagTypeAtom=true;nAtoms=1;}
     	...
     	if  (mNuclearCharge.find() && flagTypeAtom) {
    		...
    		if (mXYZ.find() &&  jobHessian) {flagGetXYZ=true; XYZLine=lineNumber; iAtom = -1; mass=0.0;}
            ...
            if (flagGetXYZ && (lineNumber>=(XYZLine+4)) ){ 
        	       ...
        	       if (buffer.length == 7) {
                      ...
                      atomXYZ[iAtom][2] = Double.parseDouble(buffer[6]); // Z
        	    	      ...
        	    	      flagGetXYZ=false; nAtoms = iAtom+1;
              	...
              	if (nAtoms>1) {
                    ...
                    for (int iat=0; iat<nAtoms; iat++){
                    	inertiaTensor[0][1] = inertiaTensor[0][1] - atomMass[iat]*atomXYZ[iat][0]*atomXYZ[iat][1];                                                                              
                     }
                    ...
                    for (int iat=0; iat<nAtoms; iat++){
                    	inertiaTensor[0][2] = inertiaTensor[0][2] - atomMass[iat]*atomXYZ[iat][0]*atomXYZ[iat][2];                                                                              
                     }
                    inertiaTensor[2][0] = inertiaTensor[0][2];
                    //YZ
                    inertiaTensor[1][2] = 0.0;
                    for (int iat=0; iat<nAtoms; iat++){
                    	inertiaTensor[1][2] = inertiaTensor[1][2] - atomMass[iat]*atomXYZ[iat][1]*atomXYZ[iat][2];                                                                              
                     }
        ...
        if (!jobHessian && (nAtoms >1) ) 
              ...
              String message = "Error in Class ChemicalSystem, in method kinpFromMOLPRO" + Constants.newLine;            
              message = message + "while checking the type of job in the outputfile " + read.getWorkFile() + Constants.newLine;
           ...
           if (!job) 
                  ...
                  /*ATOM number : needed to avoid printing Inertia moment and linearity information*/        	                     
                  // checking the detected atom number
    	       if (nAtoms==0) 
               	{write.end(); // end the file building
               	String message = "Error in Class ChemicalSystem, in method kinpFromMOLPRO" + Constants.newLine;            
               	message = message + "while reading atomic coordinates, no atom found in file " + read.getWorkFile() + Constants.newLine;
               ...
               // !! here, in MOLPRO, this rotational symmetry number is either detected or not yet supplied ... then, default is 1 in Kisthelp
    	    ...
    	    /* ROTATIONAL SYMMETRY NUMBER   IF AND ONLY IF NON-ATOMIC SYSTEM*/             
    	      ...
    	      if (nAtoms>1) {
    	       ...
    	       // first check number of freq. consistency 	    	        	    
    	            if ( (linearity && freq.size()!=(3*nAtoms-5)) || (!linearity && freq.size()!=(3*nAtoms-6)) ) {
        	            {write.end(); // end the file building
        	            String message = "Error in Class ChemicalSystem, in method kinpFromMOLPRO" + Constants.newLine;            
        	            message = message + "while reading vibrational frequencies for a system with more than one atom in file "+ read.getWorkFile() + Constants.newLine;
        	            message = message + "(linear molecule and nb of freq. !=3N-5) OR ( not linear && nb of freq. != 3N-6) detected" + Constants.newLine;
        	            JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	
              ...
              // first, remove the 6 or 5 lowest REAL frequencies (rotations, translations)
               // !! Here in ORCA, these low frequencies are not read by Kisthelp, in contrast with GAMESS OUTPUT
    		       ...
    		       for (int iFreq=0; iFreq< freq.size(); iFreq++) {
    		          ...
    		          if (freq.get(iFreq).getImagPart() == 0) {write.oneDouble(freq.get(iFreq).getRealPart());}
    		          else {write.oneString(  String.valueOf(freq.get(iFreq).getImagPart())+ "i"    );}
           ...
           /* ELECTRONIC DEGENERACY */
    	       ...
    	       if (nAtoms >1) {     
     	         ...
     	         if (linearity) {write.oneString("linear");}
     			 else {write.oneString("not linear");}
     			 write.oneString("*END");
        ...
        /**********************************/
        /* k i n p F r o m O R C A */   // build a kinp input file from data in a ORCA (from version 4.0.1.2) frequency outputfile  
        /********************************/   
            
            public  static void kinpFromOrca(ActionOnFileRead read, ActionOnFileWrite write) throws IllegalDataException, IOException { 
         ...
         // molecular properties 
          ...
          pTestNegatDecimal      = Pattern.compile("^[-][0-9]+\\.[0-9]*$");  
            ...
            while (currentLine != null) {	
                     ...
                     /* IMAGINARY VIBRATIONAL FREQUENCIES   (in ORCA, real and imaginary frequencies are reported in two different parts)   */    
             /* pay attention with ORCA: imaginary frequencies are highlighted with the "imaginary" text*/
        		mFreqImagin=pFreqImagin.matcher(currentLine);			
        		if (mFreqImagin.find()) {
                	      freqBuffer = currentLine.split(" +");
                     ...
                     /* REAL VIBRATIONAL FREQUENCIES   (in ORCA, real and imaginary frequencies are reported in two different parts)   */    
             /* pay attention with ORCA: imaginary frequencies are highlighted with the "imaginary" text*/
        		mFreq=pFreq.matcher(currentLine);			
        		if (mFreq.find()) {		
                	      freqBuffer = currentLine.split(" +");
        		...
        		// the total mass can be obtained from the atom mass sum : no need for mass matcher
        		// --> for an atom, there is no need to perform a frequency calculation within ORCA
            ...
            if (flagMass && (lineNumber>=(massLine+3)) ){  // after coordinate starting block   	
               ...
               if (buffer.length != 9) {// detect the end of the XYZ/mass section => treatment
        		...
        		/* ROTATIONAL SYMMETRY NUMBER */           
            symNumber = "1"; // !! ORCA does not yet supply the symmetry number; default is 1 here; this is to say: there is no problem
            ...
            if (!jobHessian && (nAtoms >1) ) 
               ...
               if (!job) 
        	       ...
        	       if (nAtoms==0) 
                   	{write.end(); // end the file building
                   	String message = "Error in Class ChemicalSystem, in method kinpFromOrca" + Constants.newLine;            
                   	message = message + "while reading atomic mass, no atom found in file " + read.getWorkFile() + Constants.newLine;
                   ...
                   /* ROTATIONAL SYMETRY NUMBER   */               
                   // !! here, in ORCA, this rotational symmetry number is not yet supplied ... then, default is 1 in Kisthelp
    ...
    /********************************/   
        ...
        public  static void kinpFromNwc(ActionOnFileRead read, ActionOnFileWrite write) throws IllegalDataException, IOException { 
      ...
      pTestPositDecimalWithExp = Pattern.compile("^[0-9]*\\.[0-9]*E[\\-\\+]?[0-9]+");
        ...
        while (currentLine != null) {	
                    ...
                    /* C H E C K   H E S S I A N      R U N T Y P E */
    	 	           ...
    	 	           if (mHessian.find())  {jobHessian=true;}
  

    	 	          /* Nb of A T O M S */
    	 	            mAtom = pAtom.matcher(currentLine);
     	    		    if (mAtom.find()) {flagAtom= true; atomLine=lineNumber;}
                  ...
                  /* POTENTIAL ENERGY*/
    	 	         ...
    	 	         if (mEnergy.find()) {    	 	          	  
    	 	             buffer = currentLine.split(" +");
  	 	             if (!energy.isEmpty()) {flagMoreThanOneEnerg=true;}           	      
  	 	             else {energy = buffer[5];}
    	 	         }
                 ...
                 /* VIBRATIONAL FREQUENCIES     */    
    	             /* recall that a negative number represents an imaginary number in NWCHEM outputfile ...*/   
	 	         	 	         
	     		mFreqDetect=pFreqDetect.matcher(currentLine);			
	    		    if (mFreqDetect.find()) {flagFreq= true; freqLine=lineNumber;}
	            if (flagFreq && (lineNumber>=(freqLine+3)) ){

	               	buffer = currentLine.split(" +");
	             	// detect the end of the freq section
	              	if (buffer.length <3) {flagFreq= false;}
	                if (flagFreq) {
	        	           if (Double.parseDouble(buffer[2])==0.0) { // translationnal Rotational Degree of Freedom 
	        	  	            ...
	        	  	            //message = message + "Excluding small frequency: " + buffer[2] + " cm-1 from the set of vib. frequencies" + Constants.newLine;
                            ...
                            System.out.println("Excluding small frequency: " + buffer[2] + " cm-1 from the set of vib. frequencies");
	        	        	        ...
	        	        	        else {freq.add(new Complex(Double.parseDouble(buffer[2]),0.0));}	        	        	        
    		...
    		if (mMass.find() ) {flagMass= true; massLine=lineNumber; mass=0.0;}
        if (flagMass && (lineNumber>=(massLine+3)) ){
          	
           	buffer = currentLine.split(" +");
         	// detect the end of the mass section
          	if (buffer.length !=7) {flagMass= false;}
            if (flagMass) {
		   ...
		   if (mAtomMass.find()) {flagAtomMass= true; atomMassLine=lineNumber;}
		...
		if (mLinear.find() ) {linearity=true;}
       
        
        
                 /* INERTIA MOMENTS  and molecular Linearity*/    
		// if previous inertia moments (another previous geom) already loaded, erase previous vector
    		mInertia= pInertia.matcher(currentLine);		// try to detect inertia moments of a molecule	
    		if (mInertia.find() ) {flagInertia= true; inertiaLine=lineNumber; if (inertia.size()!=0) {inertia.removeAllElements();}}
        if (flagInertia && (lineNumber>=(inertiaLine+2)) ){
        	
    		   		
    			       buffer = currentLine.split(" +");			
    		         	// detect the end of the mass section
    		          	if (buffer.length !=7) {flagInertia= false; }// even for linear molecules, 3 fields are expected in NWchem
    		            if (flagInertia) {
                   ...
                   // checking if rotational constant are decimal numbers
         		        	         ...
         		        	         if (!mTestDecimal.find()) 
         		        			       ...
         		        			       JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage,
         		        			    		   JOptionPane.ERROR_MESSAGE);				                       
         		        			       throw new IllegalDataException();}
         		        	         else {inertia.add(Double.parseDouble(buffer[2]));}
                  ...
                  symNumber = symNumberBuffer[9];
    		          ...
    		          if (!mTestInt.find()) 
    		            {write.end(); // end the file building
      			       ...
      			       String message = "Error in Class ChemicalSystem, in method kinpFromNwc" + Constants.newLine;            
      			       message = message + "while reading Rotational Symmetry Number in file " + read.getWorkFile() + Constants.newLine;
         ...
         //********************************************//
             ...
             if (flagMoreThanOneEnerg) {
  	            String message = "Warning in Class ChemicalSystem, in method kinpFromNwc" + Constants.newLine;            
  	            message = message + "Multi-Job detected, more than one total energy detected, first occurrence kept" + Constants.newLine;
  	            JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.WARNING_MESSAGE);				                             	    
              }
            ...
            if (!jobHessian && (nAtoms >1) ) 
               ...
               throw new IllegalDataException(); // if end      		  
     	       ...
     	       if (nAtoms==0) 
                    ...
                    message = message + "**XYZ format geometry** pattern not found in file " +  read.getWorkFile() + Constants.newLine;
                    JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage,
                 		   JOptionPane.ERROR_MESSAGE);				                           
                ...
                if (degen.isEmpty()){
     	            ...
     	            String message = "Warning in Class ChemicalSystem, in method kinpFromNwc" + Constants.newLine;            
     	            message = message + "Warning, **Spin multiplicity:** pattern not found in " + read.getWorkFile() + Constants.newLine;
	            ...
	            String message = "Error in Class ChemicalSystem, in method kinpFromNwc" + Constants.newLine;            
	            message = message + "rotational symmetry number not found in file " + read.getWorkFile() + Constants.newLine;
             ...
             /* FREQUENCIES  */               
     	        ...
     	        if ( (freq.size()==0) && (nAtoms>1) ) 
     	           ...
     	           String message = "Error in Class ChemicalSystem, in method kinpFromNwc" + Constants.newLine;            
     	           message = message + "vibrational frequencies not found in file "+ read.getWorkFile() + Constants.newLine;
     	           message = message + "**Normal Eigenvalue ||    Projected Derivative Dipole Moments**  pattern not found"  + Constants.newLine;
     	           JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage,
     	        		   JOptionPane.ERROR_MESSAGE);				                             	    
     	           throw new IllegalDataException();} // if end
                ...
                /* MASS   */             
       	      ...
       	      write.oneString("*MASS (in amu)");
       	      if (nAtoms>1) {write.oneDouble(mass);} else if (nAtoms==1) {write.oneDouble(massAtom1);};
       	      write.oneString("*END"); 
    	    ...
    	    /* ROTATIONAL SYMMETRY NUMBER   IF AND ONLY IF NON-ATOMIC SYSTEM*/             
    	       ...
    	       // first check number of freq. consistency 	
    	            if ( (linearity && (freq.size()!=(3*nAtoms-5))) || (!linearity && (freq.size()!=(3*nAtoms-6))) ) {
        	            ...
        	            String message = "Error in Class ChemicalSystem, in method kinpFromNwc" + Constants.newLine;            
        	            message = message + nAtoms + " atoms and " + freq.size() + " vib. freqs in " + read.getWorkFile() + Constants.newLine;
        	            message = message + "(linear molecule and nb of freq. !=3N-5) OR ( not linear && nb of freq. !=3N-6)"  + Constants.newLine;
        	            JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage,
        	            		JOptionPane.ERROR_MESSAGE);				                             	    
           ...
           write.oneString(degen);
     			 ...
     			 if (!linearity) { // three components to be written
     			    for (int iInertia=0; iInertia< 3; iInertia++) {	
     			    	   ...
     			    	   if ((Double)inertia.get(iInertia)==0.0) {
     			    		        ...
     			    		        String message = "Error in Class ChemicalSystem, in method kinpFromNwc" + Constants.newLine;            
     			    		        message = message + "non linear system but one inertia moment is null ... in " + read.getWorkFile() + Constants.newLine;
     			    		        JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage,JOptionPane.ERROR_MESSAGE);	
     			    		        					                             	        		         	        	    
     		          	        throw new IllegalDataException();} // if end       			    		   
     			    	   
     			    	   else {write.oneDouble(Constants.convertcm_1ToAmuBohr2/(Double)inertia.get(iInertia)); }		      
     			    }// for end
        	            ...
        	            if      (a==0.0) { write.oneDouble(Constants.convertcm_1ToAmuBohr2/(Double)inertia.get(1));}
        	            else if (b==0.0) { write.oneDouble(Constants.convertcm_1ToAmuBohr2/(Double)inertia.get(2));}
        	            else if (c==0.0) { write.oneDouble(Constants.convertcm_1ToAmuBohr2/(Double)inertia.get(0));}
     			 }// else end
    	       ...
    	       //  tell the user the file kinp has been built : DEPRECATED since a new kisthelpDialog asking for kinp location is now provided
    	       //String message = "Input file " + write.getWorkFile().getName() + " was successfully built";      
    	       //JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.INFORMATION_MESSAGE);	
    	    ...
    	    /********************************/   
				...
				public  static void kinpFromADF(ActionOnFileRead read, ActionOnFileWrite write) throws IllegalDataException, IOException { 
    	     ...
    	     // molecular properties 
    	        ...
    	        while (currentLine != null) {	
    	 	 	           ...
    	 	 	           if (flagZMAT && (lineNumber>=(zmatLINE+1)) ) {
    	 	 	              ...
    	 	 	              //if (currentLine.trim().isEmpty()) { // trim() remove any space or tabulation in the line
    	 	 	        	 if (currentLine.matches("^\\s*$")) { // means: at the beginning of the line: 0 or more spaces ending the line
    	 	 	        	  ...
    	 	 	        	  nAtoms = nAtoms + 1;
    	 	 	        	  mass = mass + Constants.getAtomicMass(atomName); // needed to compare the sum of reactant masses with
                                                                               // TS mass
    	 	 	              }
    	                 ...
    	                 /* VIBRATIONAL FREQUENCIES     */    
    	    	             /* recall that a negative number represents an imaginary number in ADF outputfile ...*/   
    		                ...
    		                if (flagFreq && (lineNumber>=(freqLine+1)) ){
    		                  	...
    		                  	buffer = currentLine.split(" +");
    		             	   // detect the end of the freq section
    		                 	if (buffer.length <5) {flagFreq= false;}
    		                    if (flagFreq) {   		        	              		        	           
    		        	        	        if (Double.parseDouble(buffer[2]) < 0) { // imaginary frequency	        	        	   
    		        	        	        	   freq.add(new Complex(0.0, Math.abs(Double.parseDouble(buffer[2]))));
    		        	        	        	   }
     		        ...
     		        /* MASS */ 
    	    		/* DEPRECATED, now masses are deduced from atomic name taken from z-matrix, see above */
    	    		/*
    	    		mAtomMass= pAtomMass.matcher(currentLine);
    	    		if (mAtomMass.find() ) {flagMass= true; massLine=lineNumber; mass=0.0; nAtoms=0;}
    	            if (flagMass && (lineNumber>=(massLine+2)) ){
    	                ...
    	                if (flagMass) {    	    	                              	    	              
    			               ...
    			               if ( Double.parseDouble(buffer[8]) <= 0.0  ) 
    			                  {write.end(); // end the file building
    			                   ...
    			                   message = message + "while reading molecular mass in file " + read.getWorkFile() + Constants.newLine;
    			                   message = message + "=> " +   buffer[8] + ": not a positive decimal number" + Constants.newLine;
    			                   JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage,
    			    		       JOptionPane.ERROR_MESSAGE);				                
    	                           throw new IllegalDataException();
    	           	               ...
    	           	               mass = mass + Double.parseDouble(buffer[8]); // needed to compare the sum of reactants masses wit
    	           	                                                            // TS mass
    	           	               nAtoms = nAtoms + 1;
    	                   }// end of if (flagMass)
    	                 } // end of if (flagMass && (lineNumber>=(massLine+3)) )     
    	              ...
    	              */
    	                 ...
    	                 if (flagInertia && (lineNumber>=(inertiaLine+3)) ){
    	    			       ...
    	    			       buffer = currentLine.split(" +");			
    	    		         	// detect the end of the mass section
    	    		          	if (buffer.length <3) {flagInertia= false; }// even for linear molecules, 3 fields are expected in ADF
    	    		            if (flagInertia) {   	                     
    	    		...
    	    		if (mSymNumber.find() ) {    	    		         	       
    	    		          ...
    	    		          if (!mTestInt.find()) 
    	    		            {write.end(); // end the file building
    	      			       ...
    	      			       JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage,
    	      			    		   JOptionPane.ERROR_MESSAGE);				                       
    	      			       throw new IllegalDataException();} // if  (!mTestInt.find())end
    	         ...
    	         //********************************************//
    	            ...
    	            if (!jobHessian) 
    	               ...
    	               // checking the detected atom number
    	 	           ...
    	 	           /* DEGENERACY   */  // no longer possible in ADF since if we do not find the pattern, default is degen=1             
    	    	         ...
    	    	         write.oneString("*MOMENT OF INERTIA (in Amu.bohr**2)");
    	     			 ...
    	     			 if (!linearity) { // three components to be written
    	     			    for (int iInertia=0; iInertia< 3; iInertia++) {	
    	     			    	   ...
    	     			    	   else {write.oneDouble((Double)inertia.get(iInertia)); }		      
    	        	            ...
    	        	            if      (a==0.0) { write.oneDouble((Double)inertia.get(1));}
    	        	            else if (b==0.0) { write.oneDouble((Double)inertia.get(2));}
    	        	            else if (c==0.0) { write.oneDouble((Double)inertia.get(0));}
    	     			 }// else (!linearity) end
    ...
    /**********************************/
    /* k i n p F r o m G m s 2 0 1 2 */   // build a kinp input file from data in a Gamess2012 or Gamess2013 frequency outputfile  
    /********************************/   
        
        public  static void kinpFromGms2012(ActionOnFileRead read, ActionOnFileWrite write) throws IllegalDataException, IOException { 
     ...
     // molecular properties 
      ...
      pJob                   = Pattern.compile("EXECUTION OF GAMESS TERMINATED NORMALLY");
        ...
        while (currentLine != null) {	
                 ...
                 /* VIBRATIONAL FREQUENCIES     */    
         /* pay attention with gamess: imaginary frequencies are highlighted with the "I" character after a blank character, "3000 I" i.e */
	 	         // here, all the 3N "frequencies" are read ! (even the 6 first numbers)
    		mFreq=pFreq.matcher(currentLine);			
    		if (mFreq.find()) {
            	      freqBuffer = currentLine.split(" +");
            	      int iFreq = 2; // frequencies start at field 2, but imaginary frequencies should be envisaged => consider also field 3...
            	      while (iFreq <= freqBuffer.length-1) {    
		                   ...
		                   message = message + "while reading vibrational frequencies in file " + read.getWorkFile() + Constants.newLine;
		                   message = message + "=> " +   freqBuffer[iFreq-1] + "  : not a decimal number" + Constants.newLine;
		                   JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage,JOptionPane.ERROR_MESSAGE);	
		                   ...
		                   if ( (iFreq+1)<= freqBuffer.length-1) {
                               ...
                               else freq.add(new Complex(Double.parseDouble(freqBuffer[iFreq]),0));
		                   }// end of if ( (iFreq+1)<= 
		                   else {freq.add(new Complex(Double.parseDouble(freqBuffer[iFreq]),0));}
        ...
        if (flagMass && (lineNumber>=(massLine+2)) ){
           	...
           	buffer = currentLine.split(" +");
         	// detect the end of the mass section
          	if (buffer.length !=4) {flagMass= false; nAtoms = iAtom; }
            if (flagMass) {
    	           iAtom++; 
        ...
        if (flagInertia && (lineNumber>=(inertiaLine+1)) ){
    			       ...
    			       buffer = currentLine.split(" +");			
    		         	// detect the end of the mass section
    		          	if (buffer.length !=4) {flagInertia= false; }// even for linear molecules, 3 fields are expected in GAMESS
    		            if (flagInertia) {
         		           ...
         		           for (int i=1; i<=3; i++){
         		        	        ...
         		        	        mTestDecimal = pTestDecimal.matcher(buffer[i]);
         		        	         if (!mTestDecimal.find()) 
         		        	   	      	{
         		        	        	 String message = "Error in Class ChemicalSystem, in method kinpFromGms" + Constants.newLine;            
         		        	        	 message = message + "while reading Rotational Constants in file " + read.getWorkFile() + Constants.newLine;
                  ...
                  // treatment
         		          ...
         		          if ( (a<Constants.inertiaEpsilon) &&  (Math.abs(b - c ) < Constants.inertiaEpsilon) ) 
         		             {linearity=true; inertia.add(Constants.convertGHzToAmuBohr2/b);}
         		          
         		          if ( (b<Constants.inertiaEpsilon) &&  (Math.abs(c - a ) < Constants.inertiaEpsilon) ) 
         		             {linearity=true; inertia.add(Constants.convertGHzToAmuBohr2/c);}

         		          if ( (c<Constants.inertiaEpsilon) &&  (Math.abs(a - b ) < Constants.inertiaEpsilon) ) 
         		             {linearity=true; inertia.add(Constants.convertGHzToAmuBohr2/a);}
        ...
        //********************************************//
           ...
           if (!job) 
    	    ...
    	    /* ROTATIONAL SYMMETRY NUMBER   IF AND ONLY IF NON-ATOMIC SYSTEM*/             
    	      ...
    	      if (nAtoms>1) {
    	       ...
    	       // first check number of freq. consistency 	
    	            if ( (linearity && freq.size()<5) || (!linearity && freq.size()<6) ) {
        	            ...
        	            String message = "Error in Class ChemicalSystem, in method kinpFromGms" + Constants.newLine;            
        	            message = message + "while reading vibrational frequencies for a system with more than one atom in file "+ read.getWorkFile() + Constants.newLine;
        	            message = message + "(linear molecule and nb of freq. <5) OR ( not linear && nb of freq. <6) detected" + Constants.newLine;
        	            JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	
              ...
              // first, remove the 6 or 5 lowest REAL frequencies (rotations, translations)
    	        	int nRotTrans=0;
    	        	if (linearity) {nRotTrans = 5;} else {nRotTrans=6;}
    	        for (int iRotTrans=1; iRotTrans<=nRotTrans; iRotTrans++) {	
    	  	      ...
    	  	      message = message + "Warning, removing small frequency: " + smallestCx.getRealPart() + " cm-1" + Constants.newLine;
    	  	      JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.WARNING_MESSAGE);				                             	    
...
/********************************/   
    ...
    public  static void kinpFromG09(ActionOnFileRead read, ActionOnFileWrite write) throws IllegalDataException, IOException { 
 ...
 // molecular properties
        ...
        pTestNegatDecimal      = Pattern.compile("^[-][0-9]+\\.[0-9]*$");  
	    pTestDecimal           = Pattern.compile("^[-]?[0-9]+\\.[0-9]*$"); 
	    pArchive               = Pattern.compile("1\\|1\\||1\\\\1\\\\"); // match either a Linux (\1\1) or Windows (1|1|) gaussian output 
        ...
        Boolean 				   newFreqSetFlag=true; // used to indicate whether a new frequency set is read or not during the file parsing
	    ...
	    /******************************************/   
	       ...
	       if (mArchive.find() ) {flagArchive = true; if (archive.length()!=0) {archive="";}} // reinitialize archive for a multiple-job outputfile
	       if (flagArchive) {
             ...
             /* VIBRATIONAL FREQUENCIES     */    
	     /* recall that a negative number represents an imaginary number in gaussian outputfile ...*/     
	       /* here : only 3N-6(5) numbers are read since gaussian already removed the trans+rot modes */

		mFreq=pFreq.matcher(currentLine);			
		if (mFreq.find()) {
			
			  /* Take care, if a second job is encountered and the two jobs in the file contains frequencies information */ 
		...
		if (mInertia2.find() ) {
             linearity = true;
			// if previous inertia moments (another previous geom) already loaded, erase previous vector
			if (inertia.size()!=0) {inertia.removeAllElements();}
            ...
            else {write.end(); // end the file building
            
            String message = "Error in Class ChemicalSystem, in method kinpFromG09" + Constants.newLine;            
            message = message + "while reading a rotational constant in file " + read.getWorkFile() + Constants.newLine;
            message = message + "only 5 fields were expected in line ** " + currentLine + " **" + Constants.newLine;
            JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	
            ...
            // checking if rotational constant are decimal numbers
               ...
               if (!mTestDecimal.find()) 
		          {            	   
            	   String message = "Error in Class ChemicalSystem, in method kinpFromG09" + Constants.newLine;            
            	   message = message + "while reading Rotatonal Constants in file " + read.getWorkFile() + Constants.newLine;
		...
		if (mInertia.find() ) {
			...
			inertiaBuffer = currentLine.split(" +");			
               ...
               String message = "Error in Class ChemicalSystem, in method kinpFromG09" + Constants.newLine;            
               message = message + "while reading rotational ctes in file " + read.getWorkFile() + Constants.newLine;
               message = message + "only 6 fields were expected in line ** " + currentLine + " **" + Constants.newLine;
               JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       
               throw new IllegalDataException();                            
	    ...
	    //********************************************//
      	  ...
      	  if (archive.length()==0) 
      	    ...
      	    String message = "Error in Class ChemicalSystem, in method kinpFromG09" + Constants.newLine;            
      	    message = message + "while reading electronic degeneracy in gaussian archive of " + read.getWorkFile() + Constants.newLine;
      	    ...
      	    throw new IllegalDataException();} // if end
      		 ...
      		 // First, replace all pipe (gaussian under Windows) character present in the archive string with a \ character (Linux)
             ...
             if (!mFreqKey.find()) 
 	           ...
 	           String message = "Error in Class ChemicalSystem, in method kinpFromG09" + Constants.newLine;            
 	           message = message + "while checking the \\Freq\\ keyword in the archive of " + read.getWorkFile() + Constants.newLine;
 	           message = message + "=> a frequency job is required (when gaussian is employed), even for atomic species"+ Constants.newLine;
 	           JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       
	    ...
	    if (symNumber.equals("***") && (nAtoms>1)) 
   	       ...
   	       String message = "Error in Class ChemicalSystem, in method kinpFromG09" + Constants.newLine;            
   	       message = message + "while reading rotational symmetry number for a system with more than one atom in file " + read.getWorkFile() + Constants.newLine;
	    ...
	    if ( (freq.size()==0) && (nAtoms>1) ) 
	        ...
	        if (nAtoms>1) {    
		       ...
		       for (int iFreq=1; iFreq<= freq.size(); iFreq++) {
		          ...
		          if (decimalNumber>0) {
		              write.oneString((String)freq.get(iFreq-1));}//if end
		              ...
		              write.oneString(imaginaryFreq);
			        	   ...
			        	   String message = "Error in Class ChemicalSystem, in method kinpFromG09" + Constants.newLine;            
			        	   message = message + "while reading vibrational frequencies in file "+  read.getWorkFile() + Constants.newLine;
			        	   message = message + "=> a frequency is null : " + (String)freq.get(iFreq-1) + Constants.newLine;
			        	   JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       
	        ...
	        if (degenBuffer.length>=4) {
                 ...
                 int backslash= degenBuffer[3].indexOf('\\'); // the fourth field is charge, multiplicity separated with a comma
                 ...
                 String multi = degenBuffer[3].substring(comma+1,backslash); // get the multiplicity number after the comma
                ...
                if (!mTestInt.find()) 
   	               ...
   	               String message = "Error in Class ChemicalSystem, in method kinpFromG09" + Constants.newLine;            
   	               message = message + "while reading electronic degeneracy in file "+ read.getWorkFile() + Constants.newLine;
	       ...
	       if (nAtoms >1) {     
	         ...
	         write.oneString("*MOMENT OF INERTIA (in Amu.bohr**2)");
 			 ...
 			 write.oneString("*END");
   	         ...
   	         String message = "Error in Class ChemicalSystem, in method kinpFromG09" + Constants.newLine;            
   	         message = message + "while reading electronic degeneracy from gaussian archive in " + read.getWorkFile() + Constants.newLine;
   	         message = message + "the \\\\ pattern should be found three times at least in the archive ..." + Constants.newLine;
   	         JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       	    
   	         throw new IllegalDataException();} // else end
	    }	 
          ...
          // by the "\\Version=" pattern : the subsection just before "S2=" pattern (for unrestricted
          // calculation) or the "RMSD" pattern (for restricted computations) is the energy value
          ...
          String message = "Error in Class ChemicalSystem, in method kinpFromG09" + Constants.newLine;            
          message = message + "while reading potential energy from gaussian archive in " + read.getWorkFile() + Constants.newLine;
          ...
          if (mS2.find() && mS2_1.find() && mS2A.find()) {// spin unrestricted job
        	  ...
        	  else   {energyBuffer = versionBuffer[1].split("\\\\RMSD");} // spin-restricted job
          
          if (energyBuffer.length==1) {write.end(); // end the file building
 	         String message = "Error in Class ChemicalSystem, in method kinpFromG09" + Constants.newLine;            
	         message = message + "while reading potential energy from gaussian archive in file " + read.getWorkFile() + Constants.newLine;
...
/******************/   
    ...
    public void massRead(ActionOnFileRead read) throws IllegalDataException,IOException {  
...

    	{
    		...
    		message = message + "*END keyword is missing to end the MASS section in file " + read.getWorkFile() + Constants.newLine;
...
/********************************/    // computes the ZPE property. Read must be of KISTHEP file TYPE
    ...
    public void frequenciesRead(ActionOnFileRead read) throws IllegalDataException, IOException {      
	 ...
	 if ((currentLine == null)||((currentLine.startsWith("*"))&&(!currentLine.toUpperCase().startsWith(Keywords.endOfInputSection))))
         {
		 String message = "Error in class chemicalsystem, in method frequenciesRead" + Constants.newLine;            
		 message = message + " the string read is not a complex neither '*END', but: "+currentLine + Constants.newLine;
		 JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);					        
		 throw new IllegalDataException();
         }
      ...
      hrdsBarrier     = new double[frequencyNumber]; // in J/molec, creates an array for barriers of the hindered rotors
          ...
          for (int iFreq=0;iFreq<frequencyNumber;iFreq++)    //  filling the unscaled vibFreq array by the vector
           ...
           { 
      		 ...
      		 // because java (split(" +")) will consider an additional field !! => " 3000" will return an empty string AND 3000 
      		 // thus, a buffer length of 2 instead of 1 !!
        	      ...
        	      for (int iChar=0; iChar<limit; iChar++){// search and delete possible  starting white space
        	    	    ...
        	    	    else if (currentLine.length()>1) {currentLine = currentLine.substring(1);}
        	    	    
        	      }// end of for (int iChar=0
 
        	      // test the case of a line containing only white spaces that became empty (more precisely: it remains one white space !
        	      if ( (currentLine.length()==1) && (currentLine.charAt(0)==' '))  {// case of only ONE starting blank character         		  

        	    	  String message =  "Error in class ChemicalSystem, in method frequenciesRead" + Constants.newLine;
        	    	  message = message + "while trying to read a frequency " + Constants.newLine;
        	    	  message = message + "a line contains only whitespaces in section *FREQUENCIES ... *END" + Constants.newLine;
         ...
         // in this case (HRDS), 2 values are expected instead of the simple frequency value
        	  // namely: frequency (cm-1) and  hindered rotor barrier in kJ/mol
        	     ...
        	     // System.out.println("0123456789012345");
        	      //System.out.println();
            	 freqBuffer = currentLine.split(" +");             	 
        		 unscaledVibFreq[iFreq] = Complex.parseComplex(freqBuffer[0]); // first field in the line is the frequency value (cm-1) // either real or imaginary (one component)
            	 ...
            	 if (freqBuffer.length==1){
            		 ...
            		 if (freqBuffer.length==2) {//hindered rotor treatment
                		 ...
                		 if (unscaledVibFreq[iFreq].getImagPart()!= 0.0) {
                			 String message =  "Error in class ChemicalSystem, in method frequenciesRead" + Constants.newLine;
                			 message = message + "while trying to read a frequency " + Constants.newLine;
                			 message = message + "a real number is expected for frequency for a hindered rotor treatment" + Constants.newLine;
            			 ...
            			 String message =  "Error in class ChemicalSystem, in method frequenciesRead" + Constants.newLine;
            			 message = message + "while trying to read a frequency " + Constants.newLine;
            			 message = message + "expected number of fields is 1 or 2; found : "+ freqBuffer.length + Constants.newLine;
        		 ...
        		 if (vibFreq[iFreq].getImagPart()<0.0) {
        			 ...
        			 String message =  "Error in class ChemicalSystem, in method frequenciesRead" + Constants.newLine;
        			 message = message + "while trying to read a frequency " + Constants.newLine;
        			 message = message + "The imaginary part of a frequency is negative:  " +  (String)freq.get(iFreq) + Constants.newLine;
	...
	// if and only if a session exists, get the scaling vib factor to scale the frequencies
    // else, the chemical systems will be built without any information coming from the session
          if (Session.getCurrentSession()!=null) {
        	     scaleVibFreq();  // to compute ZPE !! and scale the vibrational frequencies by the scaling factor updated in WorkSession 
          }
    ...
    public void symmetryNumberRead(ActionOnFileRead read) throws IllegalDataException, IOException {              
     ...
     try{
         symmetryNumber = Integer.parseInt(tok.nextToken());               
    	  ...
    	  message = message + "NoSuchElementException caught in Class ChemicalSystem, in Method symmetryNumberRead" + Constants.newLine;            
    	  ...
    	  message = message + "NumberFormatException caught in Class ChemicalSystem, in Method symmetryNumberRead" + Constants.newLine;            
    	  message = message + "the string read in not a integer, but data read is: "+symmetryNumber + Constants.newLine;
    	  message = message + "=>"+currentLine + Constants.newLine;
    	  JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       	    
    	  throw new IllegalDataException();             
                                       }			     	   	  	  
...
/************************/   
    ...
    public void inertiaRead (ActionOnFileRead read) throws IllegalDataException, IOException {                                               
      ...
      if ((currentLine == null)||((currentLine.startsWith("*"))&&(!currentLine.toUpperCase().startsWith(Keywords.endOfInputSection))))
      {
    	  ...
    	  message = message + "String read is not a double neither '*END', but: "+currentLine + Constants.newLine;
    	  JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       	    
      ...
      try{	
    	  ...
    	  String message = "NumberFormatException caught in Class ChemicalSystem, in Method inertiaRead" + Constants.newLine;            
...
/****************************************/    // and convert it from hartree to joule .kinp , must be of KISTHEP file TYPE
...
/****************************************************/
/* e l e c t r o n i c D e g e n e r a c y R e a d */      // to READ THE ELECTRONIC DEGENERACY, .kinp , must be of KISTHEP file TYPE
/**************************************************/     
    ...
    public void electronicDegeneracyRead(ActionOnFileRead read) throws IllegalDataException, IOException {           
      ...
      try{	
    	  ...
    	  String message = e.getMessage();
    	  message = message + "NoSuchElementException caught in Class ChemicalSystem, in Method electronicDegeneracyRead" + Constants.newLine;            
    	  ...
    	  String message = err.getMessage();
    	  message = message + "NumberFormatException caught in Class ChemicalSystem, in Method electronicDegeneracyRead" + Constants.newLine;            
...
/**********************/    
    ...
    public void linearRead(ActionOnFileRead read) throws IllegalDataException, IOException {                                                      
       ...
       currentLine = read.oneString();	       
       testOfCurrentLine = (((currentLine.toUpperCase().contains("LINEAR"))||(currentLine.toUpperCase().contains("NOT LINEAR"))));
       if (testOfCurrentLine==false)
	 {    	   
    	   String message = "Error in Class ChemicalSystem, in method linearRead" + Constants.newLine;            
    	   message = message + "String must be 'linear' or 'not linear' ...  found: "+currentLine + Constants.newLine;
    	   JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       	    
    	   throw new IllegalDataException();
	 }
       else{	                    			 
...
// for an atom, 3 section must be given: MASS, Potential energy, Electronic degeneracy
// for a molecule (linear or not linear), 7 sections must be given : MASS, Potential energy, Electronic degeneracy + 
// FREQ, INERTIA, SYM NUMBER and LINEARITY
       ...
       if (  (locatedSection.size() !=3) && (locatedSection.size() !=7)) {
    	   ...
    	   message = message + "Please supply 3 sections for an atom, or 7 sections for a molecule; please see documentation" + Constants.newLine;
...
// possible ATOMIC case detected
	 ...
	 (locatedSection.contains(Keywords.startOfDegeneracyElectronicInputSection)))  
	    {atomic = true;} // if end
		 ...
		 String message = "Error in Class ChemicalSystem, in method testCoherenceData" + Constants.newLine;            
		 message = message + "3 sections detected in .kinp data file, thus an atom is considered " + Constants.newLine;
		 message = message + "but sections should be : Mass, Electronic degeneracy and potential energy "+ Constants.newLine;
		 JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       	    
		 throw new IllegalDataException();
	 }// else end
 ...
 // the type (Integer, decimal, Complex) of the properties have been already checked into dataRead method or kinpFromGaussian
 // now, checking mathematical properties related to chemical properties (sign, imaginary part ...)
	...
	{
    	  String message = "Error in Class ChemicalSystem, in method testCoherenceData" + Constants.newLine;            
    	  message = message + "a potential energy should be negative; value read is: "+(up/Constants.convertHartreeToJoule)+ Constants.newLine;
    	  JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       	    
	...
	{
        	String message = "Error in Class ChemicalSystem, in method testCoherenceData" + Constants.newLine;            
        	message = message + "symmetry number should be positive; value read is: "+symmetryNumber+ Constants.newLine;
        	JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       	    
        	throw new IllegalDataException();
	}
        ...
        {
        	 String message = "Error in Class ChemicalSystem, in method testCoherenceData" + Constants.newLine;            
        	 message = message + "Only one moment of inertia is expected for a linear system ..."+ Constants.newLine;
        	 JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       	    
        	 throw new IllegalDataException();
	}	
        ...
        {
        	 String message = "Error in Class ChemicalSystem, in method testCoherenceData" + Constants.newLine;            
        	 message = message + "Three moments of inertia are expected for a non-linear system ..."+ Constants.newLine;
        	 JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       	    
        	 throw new IllegalDataException();
	    ...
	    {
          	 String message = "Error in Class ChemicalSystem, in method testCoherenceData" + Constants.newLine;            
          	 message = message + "moment of inertia should be positive; data read is: "+inertia[i]+ Constants.newLine;
          	 JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       	    
          	 throw new IllegalDataException();
	 ...
	 // for end 	
         ...
         for (int iFreq=0;iFreq<vibFreq.length;iFreq++)  { 
	        	 ...
	        	 if (currentFreq.getImagPart()<0.0) {
	        		 ...
	        		 String message = "Error in Class ChemicalSystem, in method testCoherenceData" + Constants.newLine;            
	        		 message = message + "while trying to read a frequency "+ Constants.newLine;
	        		 message = message + "The imaginary part of one frequency is negative:  " + currentFreq+ Constants.newLine;
	        		 JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       	           
	        		 throw new IllegalDataException();
	        	 }// if end
	        	 ...
	        	 if (currentFreq.getRealPart()<0.0) {
	        		 ...
	        		 String message = "Error in Class ChemicalSystem, in method testCoherenceData" + Constants.newLine;            
	        		 message = message + "while trying to read a frequency "+ Constants.newLine;
	        		 message = message + "The real part of one frequency is negative:  " +  currentFreq+ Constants.newLine;
	        		 JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       	            	       
	        		 throw new IllegalDataException();
	        	 }// if end
	        	 ...
	        	 if ((currentFreq.getRealPart()!=0.0) && (currentFreq.getImagPart()!=0.0) ){
	        		 String message = "Error in Class ChemicalSystem, in method testCoherenceData" + Constants.newLine;            
	        		 message = message + "while trying to read a frequency "+ Constants.newLine;
	        		 message = message + "Frequency must be x or iy, not x+iy; value found:  " +  currentFreq+ Constants.newLine;
	        		 JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       	                
	        		 throw new IllegalDataException();
	        	 }// if end
	  ...
	  if (nature.equals(saddlePoint1) && (negativeCurvature!=1) ) { 
 		 ...
 		 message = message + "For a TS, imaginary frequencies number should be one, but " +  negativeCurvature + " found" + Constants.newLine;
 		 JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       	                       
     	 throw new IllegalDataException();
          }// if end
	  ...
	  // from the partition, entropic  and energetic calculations, 	  
           ...
           // Rather, a reaction path point should ONLY have 1  imaginary point, but practically, from electronic structure calculations
	  // it can have 0, 1, or more than one imaginary boundary modes normal to the reaction mode 
	  // In KISTHEP, 0 or 1, or more  imaginary frequency will be accepted for a reaction path point, but
	  // a) if 0 imaginary frequency are found, then, the smallest frequency found in the .kinp file
	  // will design the reaction mode (thus removed from all partition function and rate
	  // constant treatments) !! That is the reason why, in this case, To handle a homogeneous set of data the smallest frequency is set 
	  // to imaginary number to design the reaction mode; 
	  // b) if more than 1 imaginary frequency are found, corresponding
	  // (negativeCurvature-1) imaginary frequencies are removed from statistical treatments
	  ...
	  if (nature.equals(pathPoint) && (negativeCurvature > 1) ) {  
	 		 ...
	 		 message = message + "in reaction-path points file, for one of generalized-TS, more than one imaginary frequencies found"+ Constants.newLine;
	 		 message = message + "=>         " + (negativeCurvature-1) + " imaginary frequencies removed from statistical treatments" + Constants.newLine;
	 		 JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.WARNING_MESSAGE);	       	                       
	
		  // correcting for that by removing any generalized vib. mode (transversal to the reaction path)
		  // with imaginary frequency; frequency set to 0, except the largest negative curvature that is kept
	 ...
	 if (nature.equals(pathPoint) && (negativeCurvature == 0) ) { 
 		 ...
 		 String message = "Warning in Class ChemicalSystem, in method testCoherenceData" + Constants.newLine;            
 		 message = message + "in reaction-path points file, for one of generalized-TS, none imaginary frequencies found"+ Constants.newLine;
 		 message = message + "=>  To handle a homogeneous set of data the smallest frequency is set to imaginary number to design the reaction mode";
 		 JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.WARNING_MESSAGE);	       	                       
	 ...
	 // a minimum must have all its frequencies real positive, but only a warning will be set to the user:
	 if (nature.equals(minimum) && (negativeCurvature > 0) ){
		 
		 String message = "WARNING in Class ChemicalSystem, in method testCoherenceData" + Constants.newLine;            
		 message = message + negativeCurvature + " imaginary vibrational freq. have been detected"+ Constants.newLine;
		 message = message + "For a minimum, frequencies have to be real, not imaginary";
		 JOptionPane.showMessageDialog(Interface.getKisthepInterface(),message,Constants.kisthepMessage, JOptionPane.WARNING_MESSAGE);	       	                       
		 //throw new IllegalDataException();: DEPRECATED since a warning is sent instead of an error (for students)
	 }// if end
...
/********************************************/
  ...
  public boolean getAtomic() {return atomic;}
	...
	for (int iFreq=0; iFreq< vibFreq.length; iFreq++){
		...
		if ( vibFreq[iFreq].getImagPart() ==0.0 ){
			...
			if (hrdsBarrier[iFreq] != Constants.highEnergy) {				
				...
				hrdsCorrection = Constants.R*currentFrequency/ (2 + 16*r); // thus, for r=0 (free rotor), (1/2 hNU) is removed from the ZPE
				                                                           // note that hrdsCorrection is converted to J/mol
...
public void setSymNumb(int newRotSymNumb) throws IllegalDataException{
	   ...
	   if (newRotSymNumb > 0) { symmetryNumber= newRotSymNumb;}
	   else {
...
public void setSymNumbToInitialValue() {
 ...
 public void setElecDegener(int newElecDegener) throws IllegalDataException{
	   if (newElecDegener > 0) { elecDegener= newElecDegener;}
	   else {		   
...
public double getInitialMass() {
 ...
 public void setInertia(double newInertia, int inertiaIndex) throws IllegalDataException{
   ...
   if ( (newInertia > 0) && ( (inertiaIndex==0) || (inertiaIndex==1) || (inertiaIndex==2) ) ) {inertia[inertiaIndex]=newInertia; }
   else {	   
	   String message = "Error in Class ChemicalSystem, in method setInertia" + Constants.newLine;            
	   message = message + "Attempt is done to set inertia <= 0, or inertia index out of range 0,1 or 2 ..."+ Constants.newLine;
	   JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);	       	                       
	   throw new IllegalDataException();
   }
...
public void setInertiaToInitialValue() {
	...
	for (int i=0; i<inertia.length; i++) {inertia[i] = initialInertia[i];}	
...
// with indexes correspondings to the indexes in the original array (including possinle imaginary frequencies) 
public Vector getVibRealFreqInCM_1() throws runTimeException{
  
 Vector result = new Vector();
 String[] realFreq      = new String[vibFreq.length]; //  real frequency at position realFreqIndex in the original array
...
//RETURNS all Imaginary vib freq SCALED (!!) in CM-1, converted in negative numbers, and sorted in ascendant order
...
String[] realFreq = new String[initialUnscaledVibFreq.length]; //  real frequency at position realFreqIndex in the original array UNSCALED !
int iRealFreq=0;
for (int ifreq=0; ifreq<initialUnscaledVibFreq.length; ifreq++) {

      if (initialUnscaledVibFreq[ifreq].getImagPart()==0.0) {    	   
        realFreq[iRealFreq]     = Maths.format(initialUnscaledVibFreq[ifreq].getRealPart()*Session.getCurrentSession().getScalingFactor()/Constants.convertCm_1ToKelvin,"0.0");
        iRealFreq++;}
}   
return realFreq; 
...
/************************/
// replace a vib frequency at the given index by a real frequency in the UNSCALED VIB ARRAY !
...
public void setUnscaledVibToInitialValue() {
	...
	for (int i=0; i<initialUnscaledVibFreq.length; i++) {unscaledVibFreq[i] = (Complex) initialUnscaledVibFreq[i].clone();}	
...
if (!atomic) {     
   ...
   if (!linear) { // three components to be written
	 ...
	 /* LINEARITY */
	 write.oneString("*LINEAR");			 		 
   if (linear) {write.oneString("linear");}
	 else {write.oneString("not linear");}
	 write.oneString("*END");
