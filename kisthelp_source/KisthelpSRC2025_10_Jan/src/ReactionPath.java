...
/* C O N S T R U C T O R 1*/
    ...
    public ReactionPath (double T) throws CancelException, IOException, IllegalDataException{
    	...
    	// could be interesting to order the path points in increasing order for example (in view of BWK tunneling calculation)

    	testCoherenceData(); // check that each point along the reaction path has the same number of frequencies ..
...
// of frequencies, inertia moments, the same mass ...
    ...
    // check that the number of frequencies for each point is the same
        ...
        if (currentPoint.getVibFreedomDegrees() != freqNb) {
        	...
        	message = message + "path point "+(iPoint+1) + " has " +  currentPoint.getVibFreedomDegrees() + " frequencies ..."+ Constants.newLine;
    ...
    // check that the mass is the same     
        if (currentPoint.getMass() != mass) {
        	...
        	message = message + "path point "+(iPoint+1) + " mass is: " +  currentPoint.getMass() + " ..."+ Constants.newLine;
        	message = message + "Should be the same"+ Constants.newLine;
        	JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);				                           
        	throw new IllegalDataException();       		  
     ...
     // check that electronic degeneracy is the same	
        if (currentPoint.getElecDegener() != elecDegener) {
        	...
        	message = message + "path point "+(iPoint+1) + " electronic degeneracy is: " +  currentPoint.getElecDegener() + " ..."+ Constants.newLine;
        	message = message + "Should be the same"+ Constants.newLine;
        	JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);				                           
        	throw new IllegalDataException();       		  
     ...
     // check that there is the same number of inertia moments
        if (currentPoint.getInertia().length != inertia.length) {
        	...
        	message = message + "path point "+(iPoint+1) + " inertia array length is: " +  currentPoint.getInertia().length + " ..."+ Constants.newLine;
        	message = message + "Should be the same"+ Constants.newLine;
        	JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);				                           
        	throw new IllegalDataException();       		  
   ...
   /* running over the trajectory vector to find the transition state */
      ...
      if (thereIsATransitionState) 
      ...
      else
       {
          ...
          message = message + "no reaction coordinate = 0.0 found in the provided reaction path data" + Constants.newLine;
          JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);				                           
          throw new IllegalDataException();       		  
       }  // if end
    ...
    public ReactionPathPoint getTs() { return ts;}
 ...
 // ONLY a file of type "KISTHELP input" must be READ in this case:
     File temporyFileName = KisthepDialog.requireExistingFilename(question,new KisthepInputFileFilter(Constants.kInpFileType));    
     ...
     if (currentLine==null) {
         ...
         message = message + "while reading the first line of file " + read.getWorkFile().getName() + Constants.newLine;
     ...
     while (currentLine!=null){
	    ...
	    // read the reaction coordinate and next read all the data concerning this point		
    	 ...
    	 { 
    		 ...
    		 message = message + "currentline should be "+ Keywords.startOfReactionCoordinateInputSection+ Constants.newLine;
    		 message = message + "but is: " + currentLine+ Constants.newLine;
    		 JOptionPane.showMessageDialog(null,message,Constants.kisthepMessage, JOptionPane.ERROR_MESSAGE);				                           
    		 throw new IllegalDataException();    		  
    	 ...
    	 if (!currentLine.toUpperCase().startsWith(Keywords.endOfInputSection))  
    	 {
    		 ...
    		 String message = "Error in class ReactionPath, in method dataRead" + Constants.newLine;            
    		 message = message + "while reading the endOfInputSection keyword following the reaction coordinate" + Constants.newLine;
	...
	else {setOfPoints.add(new ReactionPathPoint(ChemicalSystem.pathPoint, T, read, currentReactionCoordinate)); 
	}// else end
