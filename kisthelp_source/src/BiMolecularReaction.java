...
/*! C O N S T R U C T O R 1*/
    ...
    public BiMolecularReaction (double T, String kineticLevel) throws CancelException, runTimeException, IOException, IllegalDataException {
...
//! check that reactants and TS (and path points) are compatible points
    ...
    ReactionPathPoint currentPoint;
     ...
     if (reactant1.getAtomic()){testLinearR1=3;}
     else if (reactant1.getLinear()){testLinearR1=1;}
     
     if (reactant2.getAtomic()){testLinearR2=3;}
     else if (reactant2.getLinear()){testLinearR2=1;}
     ...
     for (int iPoint=0; iPoint<=path.getTrajectory().size()-1; iPoint++) {
	    ...
	    testLinear=0;
        if (currentPoint.getAtomic()){testLinear=3;}
        else if (currentPoint.getLinear()){testLinear=1;}
        ...
        if ( Math.abs( (massR1+massR2) -  mass) > Constants.massEpsilon)  {
        	...
        	String message = "exception caught in Class BimolecularReaction, in method testCoherenceData"+ Constants.newLine;
 	 ...
 	 //! freqNb(TS) = freqNb(R1+R2) + 6 + [testLinear(TS) - (testLinear(R1) + testLinear(R2)) ]
 	 //! with testLinear = 1 if linear,   testLinear=0 if non-linear, testLinear=3 if atomic
        
        // explanation of the following formula:
        // the regular case is : nothing is linear nor atomic, then we goes from the situation of
        // two reactants having : (3N1 - 6) vib freq. and 3 global rot and 3 global Trans 
        ...
        // without linearity, we have to substract 1 (testLinear(R1) contribution) to freqNb(R1+R2)+6
        ...
        // two reactants having : (3N1 - 3) vib freq.  and 3 global Trans for the atom (with 3N1-3 = 0 here)
        //                        (3N2 - 6) vib freq. and 3 global rot and 3 global Trans 
        // to a TS       having : 3N1+3N2-6 vib freq. and 3 global rot and 3 global Trans 
        // --> we see that 3N1+3N2-6 = freqNb(R1+R2) + 6 -3 --> a contribution of -3 must be taken into account for each atomic reactant

        expectedFreqNb = freqNbR1 + freqNbR2 + 6 + ( testLinear - (testLinearR1 + testLinearR2) )  ;
 	if ( freqNb !=   expectedFreqNb ) {
 		
 		String message = "exception caught in Class BimolecularReaction, in method testCoherenceData"+ Constants.newLine;
 		message = message +  "reactant1 + reactant2 have " + (freqNbR1 + freqNbR2) + " frequencies"+ Constants.newLine;
...
"k(cm3/molec/s)", "A(cm3/molec/s)", "Ea(kJ/mol)", "k_Arrhenius(cm3/molec/s) 2-parameters fit (k=A.exp(-Ea/RT))"};
...
"k(cm3/molec/s)", "A", "n", "Ea(kJ/mol)", "k_Arrhenius(cm3/molec/s) 3-parameters fit (k=A.T^n.exp(-Ea/RT))"};
...
// next, prepare display of Q, E, ... information   	
        ...
        {"Q(reactant1)", Maths.format(reactant1.getZTrans()/Constants.NA,"0.00E00"),
        	Maths.format(reactant1.getZVib(),"0.00E00"), Maths.format(reactant1.getZRot(),"0.00E00"), Maths.format(reactant1.getZElec(),"0.000"), Maths.format(reactant1.getZTot()/Constants.NA,"0.00E00")},
        {"Q(reactant2)", Maths.format(reactant2.getZTrans()/Constants.NA,"0.00E00"),
            	Maths.format(reactant2.getZVib(),"0.00E00"), Maths.format(reactant2.getZRot(),"0.00E00"), Maths.format(reactant2.getZElec(),"0.000"), Maths.format(reactant2.getZTot()/Constants.NA,"0.00E00")},
    ...
    /*****************************/   
    	   if (k.getKineticLevel().equals("vtst") || k.getKineticLevel().equals("vtst_w") || k.getKineticLevel().equals("vtst_eck")) {
    		   ...
    		   Object[][] data2 = {
    				   ...
    				   {"k (cm3/molec/s)", Maths.format(((RateConstantTST)k).getValueTST()*getTunnelingFactor(),"0.0000E00"), Maths.format(k.getValue(),"0.0000E00")}, 
...
//-----------------   create a DisplayPanel
	   ...
	   titleArea.setText("Bimolecular reaction at "+Maths.format(T,"0.00")+" K and "+Session.getCurrentSession().getUnitSystem().convertToPressureUnit(P)+" "+Session.getCurrentSession().getUnitSystem().getCurrentPressureUnit() + 
               "(scaling Factor = "+Session.getCurrentSession().getScalingFactor() + " - reaction path degeneracy = "+ statisticalFactor +")");
	 ...
	 //----------------- add a new plot2DPanel with deltaG(T)/or Up=f(IRC), or deltaH0K
  ...
  if (this.getRateConstant().getKineticLevel().contains("vtst")){
	  ...
	  for (int iCoord=0; iCoord < traj.size();iCoord++ ) {
		  ...
		  y3[iCoord] =  (GTS.getUp()+GTS.getZPE() - (reactant1.getUp() + reactant1.getZPE() + reactant2.getUp() + reactant2.getZPE() ))  /1000; // in kJ
	  }
...
/*         SET TITLE               */
/**********************************/    
   writeResults.oneString("         Bimolecular reaction at "+Maths.format(T,"0.00")+" K and "+Session.getCurrentSession().getUnitSystem().convertToPressureUnit(P)+" "+Session.getCurrentSession().getUnitSystem().getCurrentPressureUnit());
  ...
  else if  (k.getKineticLevel().equals("vtst") || k.getKineticLevel().equals("vtst_w") || k.getKineticLevel().equals("vtst_eck")) { 
	  writeResults.oneString("Property, Value (TS at first-order saddle point), Value (TS at \u0394 G\u00B0 Max)");}
      else {writeResults.oneString("not tst, not tst_w,not tst_eck not vtst, not vtst_w and not vtst_eck ... calculation");}
...
/*
   writeResults.oneString("         Bimolecular reaction at "+T+" K and "+Session.getCurrentSession().getUnitSystem().convertToPressureUnit(P)+" "+Session.getCurrentSession().getUnitSystem().getCurrentPressureUnit());
   writeResults.oneString("         (scaling Factor = "+Session.getCurrentSession().getScalingFactor()
...
/*
	...
	for (int jElement = 0 ; jElement < thermoChemistryArraySize ; jElement = jElement + 1){
	 ...
	 curve1.setGraphTitle("Bimolecular Rate Constant: Arrhenius fitting [ k(T) = A exp(-Ea/RT) ]"); 	   // Enter graph title
	 ...
	 curve2.setGraphTitle("Bimolecular Rate Constant: modified Arrhenius fitting [ k(T) = A.T^n  exp(-Ea/RT) ]"); 	   // Enter graph title
...
// set Title
     ...
     "*** Ea= " +  Maths.format(Ea,   "00.00") + " kJ/mol, A= " +  Maths.format(A,  "0.0000E00") + " cm3/molec/s               ***");
     writeResults.oneString("modified  Arrhenius expression: k(T) = A T^n exp(-Ea/RT)" +
     "*** Ea= " +  Maths.format(EaEa,"00.00") + " kJ/mol, A= " +  Maths.format(AA, "0.0000E00") + " cm3/molec/s/K^n, n="+
     Maths.format(nn,"0.0000") + " ***");
     writeResults.oneString("");
     // without TUNNELING    
     if (k.getKineticLevel().equals("tst") ||  k.getKineticLevel().equals("vtst") ||  k.getKineticLevel().equals("rrkmTightTs") ) {
        writeResults.oneString(" T(K)" + '\t' + "k (cm3/molec/s)" + '\t' + "k Arrhenius" + '\t' + "k modified Arrhenius" + '\t' +
     ...
     if (k.getKineticLevel().equals("tst_w") || k.getKineticLevel().equals("tst_eck") ||  
        ...
        writeResults.oneString(" T(K)" + '\t' + "X(T)"+ '\t'+ "k (cm3/molec/s)" + '\t' + "k Arrhenius" + '\t' + "k modified Arrhenius" + '\t' +
       "delta H0(kJ.mol-1)	delta S0(J.mol-1.K-1)	delta G0(kJ.mol-1)");
     } // if end
	...
	for (int jElement = 0 ; jElement < thermoChemistryArraySize ; jElement = jElement + 1){
		...
		// according to the kinetic level, the deltaG0 or deltaG0MAx will be displayed !
...
writeResults.oneString("         Bimolecular reaction at "+T+" K and "+Session.getCurrentSession().getUnitSystem().convertToPressureUnit(P)+" "+Session.getCurrentSession().getUnitSystem().getCurrentPressureUnit());
   ...
   if (k.getKineticLevel().equals("vtst") || k.getKineticLevel().equals("vtst_w") || k.getKineticLevel().equals("vtst_eck")) {
    ...
    writeResults.oneString("P ("+ Session.getCurrentSession().getUnitSystem().getCurrentPressureUnit()+")	k (cm3/molec/s)	");
...
if (thermoChemistryArraySize <= Constants.minArraySize) {
    ...
    String message = "Error in Class BimolecularReaction, in method getGraphicsResults(Temp)" + Constants.newLine;            
...
// fills the arrays for, free energy, enthalpy, entropy
		...
		if (this.getRateConstant().getKineticLevel().contains("_")){ // only tunneling effect TST/tunnel 
			...
			logKArray2[y][jElement] = Math.log(((RateConstantTST)k).getValueTST()*this.getTunnelingFactor()) / Math.log(10.0) ; // if VTST => tunneling = 1

			KArray2[x][jElement] = temperature;// we keep the Kelvin unit here !
			KArray2[y][jElement] = ((RateConstantTST)k).getValueTST()*this.getTunnelingFactor(); // if VTST => tunneling = 1
...
// build graphics  with three plots       
        ...
        dataToplot = new DataToPlot("k"+k.getKineticLevel()+" vs k_Arrhenius", title, v, labelAbscissa,labelOrdinate, w, "0.0", "0.00E00") ;
        graphicVector.add(dataToplot);
...
// build graphics
        ...
        labelAbscissa =  "Temperature ("+tempSymbol+")";
        labelOrdinate = "standard  Activation Entropy (J/mol/K)" ;// we keep J/mol/K unit for S (calculates as it) even though it can be plotted versus temperature in celsius
...
public String getTitle() {return "BIMOLECULAR REACTION";}
